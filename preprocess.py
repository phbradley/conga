    ...
    for (tag, obsm_tag) in [['gex', obsm_tag_gex], ['tcr', obsm_tag_tcr]]:
        ...
        if obsm_tag is None or (skip_X_pca_tcr and tag =='tcr'):
            ...
            for nbr_frac in nbr_fracs:
                ...
                if nbr_frac == nbr_frac_for_nndists:
                    if also_calc_nndists and nbr_frac_for_nndists is not None:
                        ...
                        nndists[slot] = _calc_nndists(D, nbrs)
...
def distcalc(args):
    ...
    D = np.argsort(cdist(X[pos[0]:pos[1], :], X),
                   kind='mergesort')  #cdist(X[pos[0]:pos[1], :], X)
    print('done ' + uid)
    np.savetxt('tmp_nbr/' + uid + '.csv',
               D[:, :int(X.shape[0] * nbr_frac) + 1].astype(int),fmt='%i',
               delimiter=",")
    return uid + '.csv'

def calc_nbrs_futures( adata, obsm_tag, nbr_fracs, target_N_for_batching ):
    ...
    for bb in range(num_batches):
        ...
        b_stop = min(N, (bb + 1) * batch_size)
        batch_indices = np.arange(b_start, b_stop)
        inds.append(((b_start, b_stop), obsm_tag, X, bb, runid ,np.max(nbr_fracs))) #calculate the largest nbr fraction in batches
...
def calc_nbrs_batched(
    ...
    if heavy_mode:
        ...
        calc_nbrs_futures(adata, obsm_tag_gex, nbr_fracs, target_N_for_batching )
        calculate_tcrdist_nbrs( 
            adata, nbr_fracs, return_array = False, 
            nbr_frac_for_nndists = nbr_frac_for_nndists, 
            tmpfile_prefix='tmp_nbr/tcr_nbr', remove_tmp_files =False)
    else:
        for (tag, obsm_tag) in [['gex', obsm_tag_gex], ['tcr', obsm_tag_tcr]]:
            if obsm_tag is None or (use_exact_tcrdist_nbrs and tag =='tcr'):
                calculate_tcrdist_nbrs( 
                    adata, nbr_fracs,  
                    return_array = False, nbr_frac_for_nndists = nbr_frac_for_nndists, 
                    tmpfile_prefix='tmp_nbr/tcr_nbr', 
                    remove_tmp_files =False)
            else:
                calc_nbrs_futures(adata, obsm_tag, nbr_fracs, target_N_for_batching )

        all_nbrs, nndists_gex, nndists_tcr = reconstruct_nbr_dict( 
            adata, nbr_fracs, path = 'tmp_nbr', 
        ...
        try:
            ...
            print("Keeping 'tmp_nbr', files still remaining in directory.")
