...
def rgb_from_fraction( fraction ):
    ## this is like the 'hot-to-cold' colormap except green only goes up to 200...
...
jet_colors = [ "#00007F", "#0000FF", "#007FFF", "#00FFFF", "#7FFF7F", "#FFFF00", "#FF7F00", "#FF0000", "#7F0000" ]
jet_color_tuples = []
for c in jet_colors:
    jet_color_tuples.append( tuple( ( float( int( x,16 ) ) for x in [ c[1:3], c[3:5], c[5:7] ] ) ) )

def interpolate_color_tuples( interp, a, b ):
    red   = max(0, min( 255, int( floor( a[0] + interp * (b[0]-a[0]) ) ) ) )
    green = max(0, min( 255, int( floor( a[1] + interp * (b[1]-a[1]) ) ) ) )
    blue  = max(0, min( 255, int( floor( a[2] + interp * (b[2]-a[2]) ) ) ) )
    return '#{:02x}{:02x}{:02x}'.format(red,green,blue)

def rgb_from_fraction_jet( fraction ):
    num_zones = 8
    assert len(jet_colors) == num_zones+1
    zone = int( floor( num_zones*fraction ) )
    if zone<0:
        return jet_colors[0]
    elif zone>=num_zones:
        return jet_colors[-1]
    else:
        interp = num_zones*fraction - zone #between 0 and 1
        return interpolate_color_tuples( interp, jet_color_tuples[zone], jet_color_tuples[zone+1] )
...
def line( p0, p1, line_width, color, dashed=False ):
    stroke_dasharray_style= ';stroke-dasharray:5,5' if dashed else ''
    return '<line x1="{:.3f}" y1="{:.3f}" x2="{:.3f}" y2="{:.3f}" style="stroke:{};stroke-width:{}{}"/>\n'\
        .format( p0[0], p0[1], p1[0], p1[1], color, line_width, stroke_dasharray_style )
...
def create_file( cmds, width, height, filename, create_png = False, background_color=None, use_xlink=False,
    ...
    if create_html:
        ...
        <title>{} {}</title>
        <div class="page">
        """.format(filename.split('/')[-1], pagename ) )

        out.write('{}\n<br>\n'.format(filename))
        out.writelines(open(filename,'r').readlines())
        out.write('<br>\n')
        out.write('</div>\n')
        out.close()
...
class SVG_tree_plotter:
    def __init__( self, rgb_from_fraction_func = rgb_from_fraction ):
        self.cmds = []
        self.rgb_from_fraction_func = rgb_from_fraction_func
    ...
    def make_line( self, p0, p1, line_width, normalized_score, extra_tag=None, color=None ):
        ...
        if normalized_score==None:
            ...
            color = self.rgb_from_fraction_func( normalized_score )
    ...
    def make_text( self, text, p, fontsize, extra_tag=None ):
        color = 'black'
        ## shift so that p is the lower left
        textwidth = fontsize*0.573 * len(text)
...
def color_stack( upper_left, lower_right, letters, colors, values ):
    ...
    for (letter, color, value) in zip( letters, colors, values ):
