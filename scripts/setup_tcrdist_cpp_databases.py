...
for organism in all_genes:
    ...
    for aa in amino_acids:
        out.write('AAdist {} {}\n'.format(aa, ' '.join(['{:.3f}'.format(
            bsd4[(aa,x)]) for x in amino_acids])))

    rep_dists = compute_all_v_region_distances(organism)
    for ab in 'AB':

        #v_genes = [ x for x in rep_dists.keys() if x[2] == ab ]
        v_genes = sorted([ x for x,y in all_genes[organism].items()
                           if y.chain == ab and y.region == 'V' ])
        print('num V{} genes {}'.format(ab, len(v_genes)))

        out.write('num_V{}_genes {}\n'.format( ab, len(v_genes) ))

        for v1 in v_genes:
            out.write('V{}dist {} {}\n'.format(ab, v1, ' '.join(['{:.3f}'.format(
                rep_dists[v1][x]) for x in v_genes])))
    out.close()
